...
from services.retrieval import retrieve_from_pinecone, format_retrieved_chunks
from services.generation import generate_response_with_llm, summarize_with_heuristics, summarize_with_local_model,summarize_with_openrouter
...
def preprocess_context(matches):
    """
    Combine and preprocess retrieved chunks for the response generation.
    """
    combined_context = "\n".join(
        match["content"].strip() for match in matches[:3] 
        # if match["content"]
    )
